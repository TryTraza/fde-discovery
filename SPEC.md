...
| 2026-04-15 | Traza alignment refactor | Adopted Traza CONTRIBUTING.md standards: module-first architecture (10 domains under `src/modules/`), `lib/api-client.ts` with typed errors, 11 service classes replacing ~50 inline `fetch()` calls, KEY factories + MATCH predicates in `lib/swr-keys.ts`, native HTML → shadcn primitives, Prettier no-semi sweep across `src/`. 250 new tests for services + swr-keys; 712 tests passing total. Old paths removed (no shims). Zero behavior changes — pure restructure. | Loose `any` types preserved on `useProcess`/`useSession` returns; tightening would balloon the diff. |
