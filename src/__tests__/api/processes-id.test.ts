...
describe('GET /api/clients/[id]/processes/[processId]', () => {
  ...
  it('returns 404 when process belongs to different client (IDOR)', async () => {
    setupClerkMocks({ isAuthenticated: true });
    vi.mocked(getProcessWithModel).mockResolvedValue({ ...fakeProcessWithModel, clientId: 'other-client' } as any);
    const req = createRequest('GET', 'http://localhost/api/clients/client-1/processes/proc-1');
    const res = await GET(req, withParams({ id: 'client-1', processId: 'proc-1' }));
    expect(res.status).toBe(404);
  });
...
describe('PATCH /api/clients/[id]/processes/[processId]', () => {
  ...
  it('returns 403 for viewer role', async () => {
    ...
    const req = createRequest('PATCH', 'http://localhost/api/clients/client-1/processes/proc-1', { name: 'Updated' });
  ...
  it('returns 400 for malformed JSON', async () => {
    setupClerkMocks({ isAuthenticated: true, publicMetadata: { role: 'admin' } });
    const req = createBadJsonRequest('PATCH', 'http://localhost/api/clients/client-1/processes/proc-1');
    const res = await PATCH(req, withParams({ id: 'client-1', processId: 'proc-1' }));
    expect(res.status).toBe(400);
  });
  ...
  it('returns 404 for nonexistent process', async () => {
    ...
    const req = createRequest('PATCH', 'http://localhost/api/clients/client-1/processes/bad-id', { name: 'Updated' });
  ...
  it('returns 200 with updated process on success', async () => {
    ...
    const req = createRequest('PATCH', 'http://localhost/api/clients/client-1/processes/proc-1', { name: 'Updated Name' });
    const res = await PATCH(req, withParams({ id: 'client-1', processId: 'proc-1' }));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('returns 422 for invalid status transition (draft → locked)', async () => {
    setupClerkMocks({ isAuthenticated: true, publicMetadata: { role: 'admin' } });
    vi.mocked(getProcessById).mockResolvedValue({ ...fakeProcess, status: 'draft', clientId: 'client-1' } as any);
    const req = createRequest('PATCH', 'http://localhost/api/clients/client-1/processes/proc-1', { status: 'locked' });
    const res = await PATCH(req, withParams({ id: 'client-1', processId: 'proc-1' }));
