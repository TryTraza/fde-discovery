...
const validSteps = [
  { id: 's1', name: 'Step 1', description: 'First', order: 1, confidence: 'confirmed', systems: [], edgeCases: [], notes: '' },
  { id: 's2', name: 'Step 2', description: 'Second', order: 2, confidence: 'inferred', systems: [{ name: 'SAP', confirmed: false, detailNotes: '' }], edgeCases: [], notes: 'Important' },
];
...
describe('PATCH /api/clients/[id]/processes/[processId]/steps', () => {
  ...
  it('returns 401 without auth', async () => {
    ...
    const res = await PATCH(createRequest({ steps: validSteps }), withParams({ id: 'c1', processId: 'p1' }));
  ...
  it('returns 404 for nonexistent process', async () => {
    ...
    const res = await PATCH(createRequest({ steps: validSteps }), withParams({ id: 'c1', processId: 'bad' }));
  ...
  it('returns 400 for invalid step data', async () => {
    setupClerkMocks({ isAuthenticated: true, publicMetadata: { role: 'admin' } });
    vi.mocked(getProcessById).mockResolvedValue(fakeProcess as any);
    const res = await PATCH(createRequest({ steps: [{ bad: 'data' }] }), withParams({ id: 'c1', processId: 'p1' }));
    expect(res.status).toBe(400);
  });
