...
vi.mock('@/lib/utils/merge-process-model', () => ({
  mergeSteps: vi.fn((_c: any, s: any) => s.map((x: any, i: number) => ({ id: `new-${i}`, ...x, order: i }))),
  mergeEdgeCases: vi.fn((_c: any, e: any) => e.map((x: any, i: number) => ({ id: `ec-${i}`, ...x }))),
  mergeSystems: vi.fn((_c: any, s: any) => s.map((x: any, i: number) => ({ id: `sys-${i}`, ...x }))),
}));
...
const fakeSynthesis = {
  ...
  steps: [{ stepId: null, name: 'Step 1', description: 'First', order: 0, confidence: 'confirmed', systems: [], changeType: 'new' }],
  edgeCases: [],
  systems: [],
...
describe('POST /api/sessions/[sessionId]/apply-synthesis', () => {
    ...
    mockTransaction.mockImplementation(async (fn: any) => {
      const tx = {
        insert: vi.fn().mockReturnValue({
          values: vi.fn().mockReturnValue({ returning: vi.fn().mockResolvedValue([fakeProcessModel]) }),
        }),
  ...
  it('applies steps only when other sections disabled', async () => {
    ...
    const { mergeSteps, mergeEdgeCases, mergeSystems } = await import('@/lib/utils/merge-process-model');

    const res = await POST(
      createRequest({ applySteps: true, applyEdgeCases: false, applySystems: false, applyQuestions: false }),
