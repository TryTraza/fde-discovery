...
const fakeEvents = [
  { id: EVENT_Q1, sessionId: SESSION_ID, type: 'QUESTION', label: 'Why is this step manual?', detail: null, timestamp: new Date('2026-03-20T10:00:00Z') },
  { id: EVENT_I1, sessionId: SESSION_ID, type: 'IMPLICIT', label: null, detail: 'User hesitated here', timestamp: new Date('2026-03-20T10:05:00Z') },
];
...
describe('POST /api/sessions/[sessionId]/debrief', () => {
  ...
  it('4. returns 400 when session type is not shadowing', async () => {
    vi.mocked(getSessionById).mockResolvedValue({ ...fakeShadowingSession, type: 'discovery' } as any);
    const res = await POST(createPostRequest({ items: [] }), withParams(SESSION_ID));
  ...
  it('6. returns 400 when session status is in_progress', async () => {
    vi.mocked(getSessionById).mockResolvedValue({ ...fakeShadowingSession, status: 'in_progress' } as any);
    const res = await POST(createPostRequest({ items: [] }), withParams(SESSION_ID));
    expect(res.status).toBe(400);
  });

  it('7. returns 400 when session already has debriefAnswers', async () => {
    vi.mocked(getSessionById).mockResolvedValue({ ...fakeShadowingSession, debriefAnswers: { items: [] } } as any);
    const res = await POST(createPostRequest({ items: [] }), withParams(SESSION_ID));
    expect(res.status).toBe(400);
    const body = await res.json();
  ...
  it('8. returns 400 when body fails validation', async () => {
    const res = await POST(createPostRequest({ items: [{ eventLogId: 'not-a-uuid', type: 'invalid', resolution: 'bad' }] }), withParams(SESSION_ID));
    expect(res.status).toBe(400);
    const body = await res.json();
