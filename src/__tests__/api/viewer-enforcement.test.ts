...
describe('Viewer role enforcement (403 on all write routes)', () => {
  ...
  it('POST /api/clients (create) → 403', async () => {
    const res = await createClient(jsonReq('/api/clients', 'POST', { name: 'Test', industry: 'IT' }) as any);
    expect(res.status).toBe(403);
  });

  it('DELETE /api/sessions/[sessionId] → 403', async () => {
    const res = await deleteSession(
      jsonReq('/api/sessions/s1', 'DELETE') as any,
  ...
  it('POST /api/sessions/[sessionId]/synthesize → 403', async () => {
    const res = await synthesize(
      jsonReq('/api/sessions/s1/synthesize') as any,
      { params: Promise.resolve({ sessionId: 's1' }) }
    );
    expect(res.status).toBe(403);
  });
  ...
  it('POST /api/sessions/[sessionId]/email-draft → 403', async () => {
    const res = await emailDraft(
      jsonReq('/api/sessions/s1/email-draft') as any,
      { params: Promise.resolve({ sessionId: 's1' }) }
    );
    expect(res.status).toBe(403);
  });
  ...
  it('POST /api/clients/[id]/processes/[processId]/artifacts (upload) → 403', async () => {
    ...
    const res = await uploadArtifact(req as any, { params: Promise.resolve({ id: 'c1', processId: 'p1' }) });
    expect(res.status).toBe(403);
  });
  ...
  it('POST /api/sessions/[sessionId]/apply-synthesis → 403', async () => {
    const res = await applySync(
      jsonReq('/api/sessions/s1/apply-synthesis') as any,
      { params: Promise.resolve({ sessionId: 's1' }) }
    );
    expect(res.status).toBe(403);
  });
