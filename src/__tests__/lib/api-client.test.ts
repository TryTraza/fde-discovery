...
import { apiClient, ApiError, ApiKeyMissingError, UnauthorizedError } from '@/lib/api-client'
...
describe('apiClient', () => {
  ...
  describe('get', () => {
    it('issues a GET and returns parsed JSON', async () => {
      ;(fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(mockResponse([{ id: '1' }]))

      const result = await apiClient.get<Array<{ id: string }>>('/api/clients')

      expect(fetch).toHaveBeenCalledWith('/api/clients', expect.objectContaining({ method: 'GET' }))
  ...
  describe('error mapping', () => {
    it('throws UnauthorizedError on 401', async () => {
      ...
      await expect(apiClient.get('/api/clients')).rejects.toBeInstanceOf(UnauthorizedError)
    })
    ...
    it('throws ApiKeyMissingError on 422 with NO_API_KEY error', async () => {
      ...
      await expect(apiClient.post('/api/sessions/1/synthesize')).rejects.toBeInstanceOf(
        ApiKeyMissingError
      )
    })
    ...
    it('throws ApiError with status text fallback when body has no error field', async () => {
      ;(fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(mockResponse({}, { status: 503 }))
    ...
    it('ApiError exposes the parsed error body', async () => {
      ;(fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
        mockResponse({ error: 'Validation failed', details: { name: 'required' } }, { status: 400 })
      )
