...
describe('Layer execution', () => {
  ...
  it('runs layers in parallel (total time ≈ max, not sum)', async () => {
    const slowLayer = {
      ...
      resolve: vi
        .fn()
        .mockImplementation(
          () => new Promise((r) => setTimeout(() => r({ data: {}, templateVars: {} }), 50))
        ),
    }
    const fastLayer = {
      name: 'l3-process',
      resolve: vi
        .fn()
        .mockImplementation(
          () => new Promise((r) => setTimeout(() => r({ data: {}, templateVars: {} }), 10))
        ),
    }
  ...
  it('continues when layer fails and fallbackOnLayerError=true', async () => {
    const failingLayer = {
      name: 'l1-domain',
      resolve: vi.fn().mockRejectedValue(new Error('L1 failed')),
    }
...
describe('Prompt compilation', () => {
  it('fetches Langfuse prompt with config promptName', async () => {
    ...
    await executeAI(defaultInput)
    expect(langfuse.getPrompt).toHaveBeenCalledWith('test-prompt', undefined, {
      label: 'production',
    })
  })
  ...
  it('merges layer templateVars into prompt compilation', async () => {
    ...
    expect(prompt.compile).toHaveBeenCalledWith(expect.objectContaining({ clientName: 'Acme' }))
  })
})
...
describe('Skill injection', () => {
  it('prepends system-prompt fragments before Langfuse prompt', async () => {
    ...
    vi.mocked(getAgentBySlug).mockResolvedValue(fakeConfig({ skills: ['some-skill'] }) as any)
...
describe('Tool resolution', () => {
  ...
  it('skips unknown tools with warning (does not throw)', async () => {
    ...
    vi.mocked(getTool).mockImplementation(() => {
...
describe('Mode dispatch', () => {
  ...
  it('calls streamText for streamText mode', async () => {
    vi.mocked(getAgentBySlug).mockResolvedValue(fakeConfig({ mode: 'streamText' }) as any)
...
describe('Overrides', () => {
  it('templateVars overrides appear in compiled prompt', async () => {
    ...
    expect(prompt.compile).toHaveBeenCalledWith(expect.objectContaining({ custom: 'value' }))
  })
