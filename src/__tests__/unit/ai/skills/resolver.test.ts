...
describe('resolveSkills', () => {
  ...
  it('groups system-prompt skill into systemPromptFragments', async () => {
    vi.mocked(getSkillsBySlugs).mockResolvedValue([
      { slug: 'process-archaeology', type: 'system-prompt', content: 'Archaeology content', enabled: true },
    ] as any);
  ...
  it('parses context-enrichment key=value pairs', async () => {
    vi.mocked(getSkillsBySlugs).mockResolvedValue([
      { slug: 'ctx-1', type: 'context-enrichment', content: 'key1=value1\nkey2=value2', enabled: true },
    ] as any);
  ...
  it('skips malformed context-enrichment lines (no =)', async () => {
    vi.mocked(getSkillsBySlugs).mockResolvedValue([
      { slug: 'bad', type: 'context-enrichment', content: 'no-equals-here\nkey=value', enabled: true },
    ] as any);
