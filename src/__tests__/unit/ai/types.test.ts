...
describe('resilienceConfigSchema', () => {
  ...
  it('rejects zero layerTimeout', () => {
    expect(() =>
      resilienceConfigSchema.parse({ ...validResilience(), layerTimeout: 0 })
    ).toThrow();
  });

  it('rejects negative layerTimeout', () => {
    expect(() =>
      resilienceConfigSchema.parse({ ...validResilience(), layerTimeout: -1 })
    ).toThrow();
  });

  it('rejects zero totalTimeout', () => {
    expect(() =>
      resilienceConfigSchema.parse({ ...validResilience(), totalTimeout: 0 })
    ).toThrow();
  });
...
describe('aiAgentConfigSchema', () => {
  ...
  it('accepts config with tools', () => {
    const result = aiAgentConfigSchema.parse(
      validConfig({ tools: [{ tool: 'web-search' }] })
    );
