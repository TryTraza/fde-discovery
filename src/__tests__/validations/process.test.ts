...
describe('updateStepsSchema', () => {
  it('accepts valid steps array', () => {
    const result = updateStepsSchema.safeParse({
      steps: [
        { id: 's1', name: 'Receive quote', description: 'Via email', order: 1, confidence: 'confirmed', systems: [{ name: 'Email', confirmed: true, detailNotes: '' }], edgeCases: [], notes: '' },
        { id: 's2', name: 'Review quote', description: 'Check pricing', order: 2, confidence: 'inferred', systems: [], edgeCases: [], notes: '' },
      ],
    });
    expect(result.success).toBe(true);
  });
  ...
  it('accepts steps with string systems (legacy format)', () => {
    const result = updateStepsSchema.safeParse({
      steps: [
        { id: 's1', name: 'Step 1', description: '', order: 1, systems: ['SAP', 'Email'] },
      ],
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.steps[0].systems[0]).toEqual({ name: 'SAP', confirmed: false, detailNotes: '' });
    }
  });
  ...
  it('applies defaults for optional fields', () => {
    const result = updateStepsSchema.safeParse({
      steps: [
        { id: 's1', name: 'Minimal step', order: 1 },
      ],
    });
    expect(result.success).toBe(true);
    if (result.success) {
