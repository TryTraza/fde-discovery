      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="space-y-2">
            {processes.slice(0, MAX_SHOWN).map((process: any) => (
              <Link
                key={process.id}
              ...
              >
                <div>
                  ...
                  {process.departmentTag && (
                    <span className="text-xs text-muted-foreground ml-2">{process.departmentTag}</span>
                  )}
                </div>
            ...
            <Link
              ...
              className={buttonVariants({ variant: 'ghost', size: 'sm', className: 'w-full mt-1' })}
            >
              View all processes
