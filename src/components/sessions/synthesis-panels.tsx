...
function changeTypeBadge(changeType: string) {
  switch (changeType) {
    case 'new': return <Badge variant="default" className="bg-green-600 text-xs">New</Badge>;
    case 'modified': return <Badge variant="default" className="bg-amber-500 text-xs">Modified</Badge>;
    case 'removed': return <Badge variant="destructive" className="text-xs">Removed</Badge>;
    default: return null;
  }
}
...
function priorityBadge(priority: string) {
  switch (priority) {
    ...
    case 'important': return <Badge variant="default" className="bg-amber-500 text-xs">Important</Badge>;
    case 'nice_to_have': return <Badge variant="secondary" className="text-xs">Nice to have</Badge>;
    default: return null;
  }
}
...
function confidenceBadge(confidence: string) {
  switch (confidence) {
    case 'confirmed': return <Badge variant="outline" className="text-xs text-green-600 border-green-600">Confirmed</Badge>;
    case 'inferred': return <Badge variant="outline" className="text-xs text-amber-500 border-amber-500">Inferred</Badge>;
    case 'missing': return <Badge variant="outline" className="text-xs text-red-500 border-red-500">Missing</Badge>;
    default: return null;
  }
}
...
export function SynthesisPanels({ sessionId, synthesis, mutateSession }: SynthesisPanelsProps) {
  ...
  const handleApply = async () => {
    setIsApplying(true);
    try {
      const res = await fetch(`/api/sessions/${sessionId}/apply-synthesis`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ applySteps, applyEdgeCases, applySystems, applyQuestions }),
      });
      if (!res.ok) {
      ...
      toast.error(err.message || 'Failed to apply synthesis');
    } finally {
      setIsApplying(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="space-y-2">
          {synthesis.steps.map((step, i) => (
            ...
            >
              <span className="font-mono text-muted-foreground w-6 text-right shrink-0">{step.order}</span>
              <div className="flex-1 min-w-0">
                ...
                {step.changeReason && (
                  <p className="text-xs text-muted-foreground italic mt-1">Reason: {step.changeReason}</p>
                )}
                {step.systems.length > 0 && (
                  <div className="flex gap-1 mt-1">
                    {step.systems.map(s => <Badge key={s} variant="secondary" className="text-xs">{s}</Badge>)}
                  </div>
                )}
              </div>
            </div>
          ...
          {synthesis.edgeCases.map((ec, i) => (
            <div key={i} className="p-2 rounded text-sm">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs">{ec.frequency}</Badge>
              </div>
          ...
          {synthesis.systems.map((sys, i) => (
            <div key={i} className="p-2 rounded text-sm">
              <div className="flex items-center gap-2">
                ...
                {sys.confirmed && <Badge variant="outline" className="text-xs text-green-600">Confirmed</Badge>}
              </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {applied && (
          <p className="text-sm text-muted-foreground">Process model updated.</p>
        )}
      </div>
    </div>
  );
            ...
            {!disabled && (
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  id={`toggle-${title}`}
                />
                <label htmlFor={`toggle-${title}`} className="text-xs text-muted-foreground cursor-pointer">
                  Include
                </label>
              </div>
            )}
