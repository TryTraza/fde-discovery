...
import { EMPTY_LAYER_RESULT, type ContextLayer, type LayerParams, type LayerResult, type L3Options } from './types';
...
function formatProcessModel(model: { steps?: any[]; systems?: any[]; edgeCases?: any[] }): string {
  ...
  if (systems.length > 0) {
    ...
    parts.push(
      systems
        .map((s: any) => `- **${s.name}**${s.confirmed ? ' (confirmed)' : ' (inferred)'}${s.detailNotes ? `: ${s.detailNotes}` : ''}`)
        .join('\n')
    );
  }
  ...
  if (edges.length > 0) {
    ...
    parts.push(edges.map((e: any) => `- ${e.description}${e.frequency ? ` (${e.frequency})` : ''}`).join('\n'));
  }
  ...
  async resolve(params: LayerParams, options?: L3Options): Promise<LayerResult> {
    ...
    if (fields === 'full') {
      const lines = [
        ...
        processWithModel.hypothesisText ? `\n### Hypothesis\n${processWithModel.hypothesisText}` : null,
      ].filter(Boolean);
