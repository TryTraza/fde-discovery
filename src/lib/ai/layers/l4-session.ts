...
import { EMPTY_LAYER_RESULT, type ContextLayer, type LayerParams, type LayerResult, type L4Options } from './types';
  ...
  async resolve(params: LayerParams, options?: L4Options): Promise<LayerResult> {
    ...
    if (eventsMode !== 'none' && events.length > 0) {
      ...
      sessionEventsSection = eventsToRender
        .map((e: any) => `- ${e.type}: ${e.label ?? '(no label)'}${e.detail ? ` (${e.detail})` : ''}`)
        .join('\n');
    }
    ...
    if (includeContacts && contacts.length > 0) {
      contactsSection = '## Session Contacts\n' + contacts
        .map((c: any) => `- **${c.name}**${c.role ? ` — ${c.role}` : ''}${c.department ? ` (${c.department})` : ''}`)
        .join('\n');
    }
