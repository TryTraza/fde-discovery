...
export function buildInterviewPrompt(params: {
  ...
  processContext: { name: string; description: string | null; hypothesisText: string | null; model: any | null; departmentTag: string | null };
  sessionType: string;
  sessionContacts: Array<{ name: string; role: string | null; department: string | null }>;
  previousAnswers: { question: string; answer: string }[];
  questionIndex: number;
}): string {
  const { client, processContext, sessionType, sessionContacts, previousAnswers, questionIndex } = params;
  ...
  const contactsContext = sessionContacts.length > 0
