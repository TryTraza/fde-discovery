...
async function seed() {
  ...
  const [proc1] = await db
    ...
    .values({
      ...
      description:
        'Team receives supplier quotes via email, compares prices in Excel, generates PO.',
      status: 'mapping',
      processTypeL1: 'procurement',
      hypothesisText:
        'The procurement team receives supplier quotes via email (Outlook). Maria and Carlos manually copy quote data into an Excel comparison spreadsheet. Once a supplier is selected, Jorge approves and a PO is generated in their ERP system.',
    })
    .returning()
  ...
  await db.insert(schema.processModels).values({
    ...
    steps: [
      {
        id: 'step_001',
        order: 1,
        name: 'Receive supplier email',
        description: 'Quote arrives via Outlook',
      ...
      },
      {
        id: 'step_002',
        order: 2,
        name: 'Open and review quote',
        description: 'Buyer opens email, checks attachments',
        confidence: 'confirmed',
        systems: ['Email'],
        nextSteps: ['step_003'],
        branchCondition: null,
        relatedEdgeCases: ['edge_001'],
        notes: '',
      },
      {
        id: 'step_003',
        order: 3,
        name: 'Enter data in comparison map',
        description: 'Copy price, terms, delivery into Excel',
        confidence: 'inferred',
        systems: ['Excel'],
        nextSteps: ['step_004'],
        branchCondition: null,
      ...
      },
      {
        ...
        description: 'Review comparison and select best option',
        confidence: 'inferred',
        systems: ['Excel'],
        nextSteps: ['step_005'],
        branchCondition: null,
      ...
      },
      {
        ...
        description: 'Send selection to director for approval',
        confidence: 'inferred',
        systems: ['Email'],
        nextSteps: ['step_006'],
        branchCondition: null,
      ...
      },
      {
        ...
        description: 'Create PO in ERP system',
        confidence: 'missing',
        systems: ['ERP'],
        nextSteps: [],
        branchCondition: null,
    ...
    edgeCases: [
      {
        id: 'edge_001',
        description: 'Supplier responds without price in email body (price only in attachment)',
        frequency: 'occasional',
        suggestedHandling: 'Open attachment to extract price',
        status: 'open',
        relatedStepId: 'step_002',
      },
    ],
    systems: [
      {
      ...
      },
      {
        ...
        details: '',
        gaps: 'Which file? Which sheet? Column structure?',
        detailNotes: '',
      },
      {
        name: 'ERP',
        ...
        role: 'PO generation',
        details: '',
        gaps: 'Which ERP system? SAP? Custom?',
        detailNotes: '',
      },
    ],
  })
  ...
  await db.insert(schema.openQuestions).values([
    {
      processId: proc1.id,
      text: 'What triggers the PO generation — is it automatic after approval?',
      priority: 'critical' as const,
      status: 'open' as const,
    },
    {
      processId: proc1.id,
      text: 'Does the manager approve every PO or only above a threshold?',
      priority: 'critical' as const,
      status: 'open' as const,
    },
    {
      processId: proc1.id,
      text: 'What naming convention does the comparison Excel use?',
