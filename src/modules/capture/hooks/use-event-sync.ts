...
export function useEventSync(sessionId: string): UseEventSyncReturn {
  ...
  const executeBatchSync = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const data = await eventsService.batchSync(
        sessionId,
        unsynced.map((ev) => ({
          sessionId: ev.sessionId,
          timestamp: ev.timestamp,
  ...
  const updateEventField = useCallback(
    (localId: string, field: 'label' | 'detail', value: string) => {
      setEvents((prev) =>
        prev.map((ev) => (ev.localId === localId ? { ...ev, [field]: value } : ev))
      )

      const event = eventsRef.current.find((ev) => ev.localId === localId)
      if (event?.serverId) {
        eventsService.updateField(sessionId, event.serverId, { [field]: value }).catch(() => {
  ...
  const removeEvent = useCallback(
    (localId: string) => {
      ...
      if (event?.serverId) {
        eventsService.delete(sessionId, event.serverId).catch(() => {
